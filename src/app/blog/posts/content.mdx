...
Replace the `public/images/avatar.jpg` file with your own avatar image. It is used on the `/sobremi` page as well as on the dynamically generated `open-graph` images.
...
Your personal details are used across the whole app to render personalized headings, labels and images. Languages are displayed on the `/sobremi` page, and location and time in the header.
...
Social links are rendered on the `/sobremi` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the Once UI documentation](https://once-ui.com/docs/icons) for more information.
